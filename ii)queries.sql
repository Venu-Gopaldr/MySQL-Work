...
select ename,job,sal*6 as halfyear from emp;

select 'hi'||ename from emp where job ='manager';
select ename||sal as full from emp;
...
#logical operators
select ename,dept_no,job from emp where dept_no = 10;
select ename,dept_no,job from emp where job = 'manager' and dept_no = 10;
select ename,dept_no,sal from emp where dept_no = 20 and sal < 3000;
select ename,sal from emp where sal > 1250 and sal < 3000;
select ename,dept_no from emp where dept_no = 20 or dept_no = 10;
select ename,job from emp where not job = 'manager';


select ename,dept_no,sal,job from emp where sal between 250 and 4000 and dept_no = 20;
...
select ename,dept_no,job from emp where job = 'manager' and (dept_no = 10 or dept_no =30);

#WAQTD name,deptno,job of EMPLOYEES working in dept 10 or 20 or 30  as a clerk
select ename,dept_no,job from emp where job = 'clerk' and (dept_no = 10 or dept_no = 20 or dept_no = 30);



#WAQTD name,job,deptno of the EMPLOYESS working as a clerk or manager in dept 10 
select ename,dept_no,job from emp where (job = 'manager' or job = 'clerk') and dept_no = 10;
...
#functions query continuations left with single row functions
...
select dept_no,count(*),avg(sal) from emp
where sal > 2000
group by dept_no;
